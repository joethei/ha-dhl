...
class DHLTracker:
    ...
    def print_shipment_info(self, shipment):
        ...
        print(f"Produkt: {shipment.get('details', {}).get('product', {}).get('productName', 'Unbekannt')}")
        print(f"Anzahl Stücke: {shipment.get('details', {}).get('totalNumberOfPieces', 'Unbekannt')}")
        print(f"Gewicht: {shipment.get('details', {}).get('weight', {}).get('value', 'Unbekannt')} {shipment.get('details', {}).get('weight', {}).get('unitText', '')}")
        print(f"Rücksendung: {'Ja' if shipment.get('returnFlag') else 'Nein'}")
        print(f"Service: {shipment.get('service')}")
        print(f"Herkunft: {shipment.get('origin', {}).get('address', {}).get('countryCode', 'Unbekannt')}")
        print(f"Ziel: {shipment.get('destination', {}).get('address', {}).get('countryCode', 'Unbekannt')}")
    ...
    def _print_shipment_history(self, shipment):
        ...
        for event in shipment.get('events', []):
            ...
            print(f"  Status: {event.get('status', '')} ({event.get('statusCode', '')})")
